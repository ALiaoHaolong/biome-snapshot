...
public class BiomeSnapshotConfigCommand {
    ...
    public int set(CommandContext<ServerCommandSource> context) {
        ...
        context.getSource().sendFeedback(() ->
                Text.translatable("command.biome-snapshot.config.biome_color_resolver.set.success", identifier.toString(), String.valueOf(color), Integer.toHexString(color).toUpperCase()), true);
        return 1;
    }
    ...
    public int remove(CommandContext<ServerCommandSource> context) {
        ...
        // 反馈
        context.getSource().sendFeedback(() ->
                Text.translatable("command.biome-snapshot.config.biome_color_resolver.remove.success", identifier.toString()), true);
        return 1;
    }
    ...
    public int get(CommandContext<ServerCommandSource> context) {
        ...
        // 反馈
        context.getSource().sendFeedback(() ->
                Text.translatable("command.biome-snapshot.config.biome_color_resolver.get", identifier.toString(), String.valueOf(color), Integer.toHexString(color).toUpperCase()), true);
        return 1;
    }
